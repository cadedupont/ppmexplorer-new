  ...
  if (error) {
    return (
      <div className="text-center text-red-500">
        {error?.message ||
          `Failed to get data for item ${id}. Please try again later.`}
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 gap-4">
              ...
              <GeoJSON
                ...
                style={{
                  color: getColorByScope(
                    data.item.location.geojson.properties.scope
                  ),
                  fillOpacity: 0.8,
                }}
              />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <InfoCard
            ...
            content={data.item.caption_en}
            icon={<Captions />}
          />
          <InfoCard
            title="Caption (Italian)"
            content={data.item.caption_it}
            icon={<Captions />}
          />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          <InfoCard
            title="Volume"
            content={data.item.volume}
            icon={<BookOpenText />}
          />
          <InfoCard
            title="Page"
            content={data.item.page}
            icon={<BookOpenText />}
          />
          <InfoCard
            title="Regio"
            content={data.item.location.regio}
            icon={<MapPin />}
          />
          <InfoCard
            title="Insula"
            content={data.item.location.insula}
            icon={<MapPin />}
          />
          <InfoCard
            title="Property"
            content={data.item.location.property}
            icon={<MapPin />}
          />
          <InfoCard
            title="Room"
            content={data.item.location.room || "N/A"}
            icon={<MapPin />}
          />
        </div>
      </div>
